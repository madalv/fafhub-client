    ...
    return(
        <Formik initialValues={{firstName: '', lastName: '', email: '', password: '', phone: '', otp: '', error: null}}
                onSubmit={(values, {setErrors}) => {userStore.register(values)
                    ...
                    .catch(error => {setErrors({error: "Registration failed: " + error.response.data.error})})
                }}
            ...
            {({handleSubmit, isSubmitting, errors, touched}) => (
                <Form className='ui form' onSubmit={handleSubmit} autoComplete='off'>
                    ...
                    {errors.firstName && touched.firstName ? (<Label style={{margin: 10}} basic color='red' content={errors.firstName}/>) : null}
                    <Field id="firstName" name="firstName" placeholder="John" type="text"/>

                    <label htmlFor='lastName'>Last Name</label>
                    {errors.lastName && touched.lastName ? (<Label style={{margin: 10}} basic color='red' content={errors.lastName}/>) : null}
                    <Field id="lastName" name="lastName" placeholder="Smith" type="text"/>

                    <label htmlFor='phone'>Phone</label>
                    {errors.phone && touched.phone ? (<Label style={{margin: 10}} basic color='red' content={errors.phone}/>) : null}
                    <Field id="phone" name="phone" type="tel" />

                    <label htmlFor='email'>Email</label>
                    {errors.email && touched.email ? (<Label style={{margin: 10}} basic color='red' content={errors.email}/>) : null}
                    <Field id="email" name="email" placeholder="john@smith.com" type="email"/>

                    <label htmlFor='otp'>OTP</label>
                    {errors.otp && touched.otp ? (<Label style={{margin: 10}} basic color='red' content={errors.otp}/>) : null}
                    <Field id="otp" name="otp" type="text"/>

                    <label htmlFor='password'>Password</label>
                    {errors.password && touched.password ? (<Label style={{margin: 10}} basic color='red' content={errors.password}/>) : null}
